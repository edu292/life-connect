...
if (isset($_GET['id'])) {
    ...
    $stmt_filhos = $conexao->prepare('DELETE FROM lote_doacao WHERE id_doacao = ?');
    $stmt_filhos->bind_param('i', $id_doacao);
    $stmt_filhos->execute();
    $stmt_filhos->close();

    // 3. EXCLUIR O REGISTRO PAI (DOACAO)
    $stmt_pai = $conexao->prepare('DELETE FROM doacao WHERE id = ?');
    $stmt_pai->bind_param('i', $id_doacao);
    $stmt_pai->execute();
